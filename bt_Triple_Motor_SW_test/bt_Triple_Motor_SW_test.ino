...
void loop() {
  ...
  if(SW1val==LOW && isMoving1)SWON1();
  if(SW2val==LOW && isMoving2)SWON2();
  if(SW3val==LOW && isMoving3)SWON3();
...
void SWON2() {
  serial.println("SW2");
  int SWval = digitalRead(SW2);
  while (SWval==LOW) {
    digitalWrite(M2IN1, LOW);
    digitalWrite(M2IN2, HIGH);
    analogWrite(M2PWM, 100);
    SWval = digitalRead(SW2);
  }
  delay(100);
  stop2();
}
...
void SWON3() {
  serial.println("SW3");
  int SWval = digitalRead(SW3);
  while (SWval==LOW) {
    digitalWrite(M3IN1, LOW);
    digitalWrite(M3IN2, HIGH);
    analogWrite(M3PWM, 100);
    SWval = digitalRead(SW3);
  }
  delay(100);
  stop3();
}
